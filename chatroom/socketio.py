...
def new_message(data):
    ...
    socketio_.emit('new_message', data, room=message.room.name)


def _leave_room(data):
    room = Room.query.get(int(data['id']))
    leave_room(room.name)
    socketio_.emit('leave_room', data['username'], room=room.name)


def _join_room(data):
    room = Room.query.get(int(data['id']))
    join_room(room.name)
    socketio_.emit('join_room', data['username'], room=room.name)


def delete_room(data):
    room = Room.query.get(int(data))
    close_room(room.name)
    socketio_.emit('delete_room', data, room=room.name)
